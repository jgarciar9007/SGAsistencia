...
class NominaHorasPDFView(LoginRequiredMixin, StaffOnlyMixin, View):
    ...
    def _build_pdf(self, request, d1: date, d2: date, rows: List[dict]) -> HttpResponse:
        ...
        body_rows = [
            [r["nombre"], r["departamento"], r["tipo"], _hhmm(r["total"])]
            for r in rows
        ]
...
class ReporteEmpleadoPDFView(LoginRequiredMixin, StaffOnlyMixin, View):
    ...
    def _build_pdf(self, request, d1: date, d2: date, meta: dict, rows):
        response = HttpResponse(content_type="application/pdf")
        filename = f"reporte_asistencia_{meta.get('nombre','trabajador')}_{d1.strftime('%Y-%m')}.pdf"
        ...
        # Tabla día a día + total de horas
        body_rows = []
        total_global = timedelta()
        for r in rows:
            fecha = r["fecha"].strftime("%d/%m/%Y")
            ent = r["entrada"].strftime("%H:%M") if r["entrada"] else ""
            sal = r["salida"].strftime("%H:%M") if r["salida"] else ""
            tot = _hhmm(r["total"])
            body_rows.append([fecha, ent, sal, tot])
            total_global += r["total"]

        # Fila de TOTAL
        body_rows.append(["", "", "TOTAL", _hhmm(total_global)])

        table = _tabla_estilizada(
            headers=["Fecha", "Entrada", "Salida", "Horas del día"],
            rows=body_rows,
            col_widths=[30 * mm, 35 * mm, 35 * mm, 35 * mm],
            style_overrides=[
                ("ALIGN", (0, 1), (0, -1), "LEFT"),   # Fecha Left
                ("ALIGN", (3, 1), (3, -1), "RIGHT"),  # Horas Right
                ("LEFTPADDING", (0, 1), (0, -1), 6),
                ("RIGHTPADDING", (3, 1), (3, -1), 6),
            ]
        )

        # Alinear horas y resaltar fila total
        n_rows = len(body_rows)  # sin contar cabecera
        last_idx = n_rows  # índice de la última fila = cabecera(0) + n_rows
        table.setStyle(TableStyle([
            ("ALIGN", (3, 1), (3, last_idx), "RIGHT"),
            ("FONTNAME", (0, last_idx), (3, last_idx), "Helvetica-Bold"),
            ("BACKGROUND", (0, last_idx), (3, last_idx), colors.HexColor("#F1F3F4")),
        ]))
...
class NominaCalculoPDFView(LoginRequiredMixin, StaffOnlyMixin, View):
    ...
    def _build_pdf(self, request, d1: date, d2: date, rows: List[dict]) -> HttpResponse:
        response = HttpResponse(content_type="application/pdf")
        response["Content-Disposition"] = f'inline; filename="nomina_{d1.strftime("%Y-%m")}.pdf"'

        doc = SimpleDocTemplate(response, pagesize=A4, leftMargin=15 * mm, rightMargin=15 * mm, topMargin=15 * mm, bottomMargin=20 * mm)

        periodo = f"PERIODO: {d1.strftime('%d/%m/%Y')}  AL  {d2.strftime('%d/%m/%Y')}"
        usuario = f"GENERADO POR: {request.user.get_username().upper()}  |  FECHA: {datetime.now().strftime('%d/%m/%Y %H:%M')}"
        story = _header_pdf_story("REPORTE DE CÁLCULO SALARIAL (NÓMINA)", periodo, usuario)
        ...
        table = _tabla_estilizada(
            headers=["Empleado", "Depto.", "S. Base", "Aus.", "Bajas", "Desc.", "Neto"],
            rows=body_rows,
            col_widths=[50 * mm, 30 * mm, 25 * mm, 15 * mm, 15 * mm, 22 * mm, 23 * mm],
            style_overrides=[
                ("ALIGN", (0, 1), (0, -1), "LEFT"), # Nombre
                ("ALIGN", (1, 1), (1, -1), "LEFT"), # Depto
                ("ALIGN", (2, 1), (-1, -1), "RIGHT"), # Números
                ("FONTSIZE", (0, 0), (-1, -1), 8),
            ]
        )
        story.extend([table, Spacer(1, 10), Paragraph("Este reporte es una estimación basada en la asistencia registrada y las bajas autorizadas.", getSampleStyleSheet()["Normal"])])
